...
namespace App\Http\Requests\User\BookbindingController;
...
class ConfirmRequest extends FormRequest
{
    ...
    {
        return [
            'appraisal_apply_ids' => ['required', 'array'],
            'appraisal_apply_ids.*' => ['required', 'integer'],
            'zip' => ['regex:/\A\d{7}\z/'],
    ...
    {
        $data = $this->all();
        if (empty($data['appraisal_apply_ids'])) {
            $validator->errors()->add('appraisal_apply_ids', '鑑定を選択してください');
