...
use App\Http\Requests\User\CheckPaymentController\ApplyRequest;
use App\Http\Requests\User\CheckPaymentController\ConfirmRequest;
...
class CheckPaymentController extends Controller
{
    ...
    public function create(): View
    {
        ...
        return view('user.check_payment.create', [
    ...
    public function confirm(ConfirmRequest $request): View
    {
        ...
        return view('user.check_payment.confirm', [
            'data'        => $data,
            'bookbinding' => Bookbinding::where('is_enabled', true)->first(),
            'appraisal'   => Appraisal::where('is_enabled', true)->first(),
        ]);
    ...
    {
        return redirect()->route('user.check_payment.create')->withInput();
    }
    ...
    public function apply(ApplyRequest $request): RedirectResponse
    {
        ...
        if ((int) $request->payment_type === AppraisalClaim::CREDIT) {
            try {
                ...
                return to_route('user.check_payment.create')->with('flash_alert', '決済に失敗しました。違うカードをお試しするか、銀行振込をご指定ください。')->withInput();
            }
        ...
        return to_route('user.check_payment.thanks');
    ...
    {
        return view('user.check_payment.thanks');
    }
}
