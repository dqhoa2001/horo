...
<div class="container">
    <x-parts.basic_card_layout>
        <x-slot name="cardHeader">
            ...
            <a href="{{ route('user.offer_appraisals.create') }}" class="btn btn-primary">作成する</a>
        </x-slot>
                ...
                <x-slot name="tbody">
                    @if($offerAppraisals->isNotEmpty())
                        @foreach($offerAppraisals as $offerAppraisal)
                            <tr>
                                <td class="text-nowrap px-2"><a href="{{ route('user.offer_appraisals.show', $offerAppraisal) }}">{{ $offerAppraisal->title }}</a></td>
                                <td class="text-nowrap px-2">{{ $offerAppraisal->created_at }}</td>
                                <td class="text-nowrap px-2"><a href="{{ route('user.offer_appraisals.edit', $offerAppraisal) }}" class="btn btn-sm btn-outline-secondary">編集</a></td>
                                <td class="text-nowrap px-2">
                                    <form action="{{ route('user.offer_appraisals.destroy', $offerAppraisal) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
