        ...
        <div class="d-flex justify-content-center align-items-center">
            <div class="logo">
                <a href="/dashboard"><img
                        src="https://hoshinomai.jp/wp/wp-content/themes/hosinomai/img/common/page_logo.svg" alt="Logo"
                        srcset="" /></a>
            </div>
        ...
        <ul class="menu">
            ...
            @foreach ($menu as $index => $item)
                @if (Route::has($item['route_name']))
                    <li
                        class="sidebar-item has-sub {{ Route::current()->getPrefix() == $item['prefix'] ? 'active' : '' }}">
                            ...
                            <ul class="submenu">
                                @foreach ($item['sub_menu'] as $submenu)
                                    @if (Route::has($submenu['route_name']))
                                        <li
                                            class="submenu-item {{ Route::currentRouteName() == $submenu['route_name'] ? 'active' : '' }}">
                                            <a href="{{ route($submenu['route_name']) }}"
                                                class="submenu-link">@lang($submenu['title'])</a>
                                        </li>
                                    @endif
                                @endforeach
                            </ul>
